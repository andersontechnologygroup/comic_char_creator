...
class Tester {
    ...
    static _verbose() {
        return !!this._env().__TEST_VERBOSE;
    }
    static _bailEnabled() {
        return !!this._env().__TEST_BAIL;
    }
    ...
    static _onFailure(div) {
        if (this._bailEnabled() && !this.bailTriggered) {
            ...
            console.error(
                "%cBAIL: Stopping on first failure.",
    ...
    static start() {
        ...
        if (this._env().__TEST_FILTER)
            console.log(`  --filter: "${this._env().__TEST_FILTER}"`);
    ...
    static getLineNumber() {
        try {
            ...
            for (let i = 2; i < Math.min(stack.length, 10); i++) {
                ...
                const isFramework = frameworkMethods.some(
                    (m) =>
                        frame.indexOf("Tester.") !== -1 &&
                        frame.indexOf(m) !== -1,
                );
    ...
    static getRenderMethodNames() {
        return this._testRegistry.map((t) => t.name).sort();
    }
    ...
    static run() {
        ...
        const allTests = this._testRegistry
            .slice()
            .sort((a, b) => a.order - b.order);
        const BENCHMARK_THRESHOLD = 900;
        const FUZZ_THRESHOLD = 800;
        const deterministicTests = allTests.filter(
            (t) =>
                t.order < FUZZ_THRESHOLD && t.iterationMode === "deterministic",
        );
        const fuzzTests = allTests.filter(
            (t) => t.order >= FUZZ_THRESHOLD && t.order < BENCHMARK_THRESHOLD,
        );
        const benchmarkTests = allTests.filter(
            (t) => t.order >= BENCHMARK_THRESHOLD,
        );
        ...
        if (this._verbose()) {
            console.log(
                `  ${deterministicTests.length} deterministic tests (run once)`,
            );
            if (fuzzTests.length > 0)
                console.log(
                    `  ${fuzzTests.length} fuzz test(s) × ${fuzzIters} iteration(s)`,
                );
            if (benchmarkTests.length > 0)
                console.log(`  ${benchmarkTests.length} benchmark test(s)`);
        }
