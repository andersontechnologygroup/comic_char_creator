...
if (coverageMode) {
  ...
  if (childArgs.length > 0) console.log('  Forwarding args: ' + childArgs.join(' ') + '\n');

  // Use --exclude to only show our project files (not node_modules)
  const c8Cmd = ['npx c8 --reporter=text --reporter=text-summary --exclude="node_modules/**"', '"' + process.execPath + '"', '"' + __filename + '"', ...childArgs.map(a => '"' + a + '"')].join(' ');
  try {
...
for (let i = 0; i < args.length; i++) {
  if (args[i] === '--filter' && i + 1 < args.length) {
    ...
    console.log('');
    console.log('Options:');
    console.log('  --filter <pattern>  Only run test methods matching <pattern>');
    console.log('  --verbose           Show each assertion result');
    console.log('  --bail              Stop on the first failure');
    console.log('  --iterations <n>    Run fuzz tests n times (default: 100, filtered: 1)');
    console.log('  --benchmark         Run performance benchmark after tests');
