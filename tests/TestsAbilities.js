...
Tester._runPrimaryAbilityTests = (gen, config) => {
  ...
  const physRolls = rolls.physical || { Fighting: 70, Agility: 60, Strength: 50, Endurance: 40 };
  const mentRolls = rolls.mental || { Reason: 30, Intuition: 20, Psyche: 10 };

  PHYSICAL_ABILITIES.forEach(a => {
    gen.physicalAbilityRolls[0][a] = physRolls[a] !== undefined ? physRolls[a] : 50;
  });
  if (rolls.physicalRow1) {
    PHYSICAL_ABILITIES.forEach(a => {
      gen.physicalAbilityRolls[1][a] = rolls.physicalRow1[a] !== undefined ? rolls.physicalRow1[a] : 50;
    });
  }
  MENTAL_ABILITIES.forEach(a => {
    gen.mentalAbilityRolls[a] = mentRolls[a] !== undefined ? mentRolls[a] : 50;
  });
  ...
  if (config.expectedRows) {
    for (const rowDef of config.expectedRows) {
      ...
      let allValid = true;
      const checkAbilities = rowIdx === 0 ? PHYSICAL_ABILITIES : PHYSICAL_ABILITIES;
      ...
      Tester.assert(allValid,
        `Primary Ability Rolls (${config.label} row${rowIdx}): All stats valid.`);

      for (const exp of abilities) {
        Tester.assertEquals(exp.rank, charStats.primaryAbilities[rowIdx][exp.ability].rank,
          `Primary Ability Rolls (${config.label} row${rowIdx}): ${exp.ability} rank.`);
        Tester.assertEquals(exp.num, charStats.primaryAbilities[rowIdx][exp.ability].number,
    ...
    if (config.expectedMental) {
      for (const exp of config.expectedMental) {
        Tester.assertEquals(exp.rank, charStats.primaryAbilities[0][exp.ability].rank,
          `Primary Ability Rolls (${config.label} mental): ${exp.ability} rank.`);
        Tester.assertEquals(exp.num, charStats.primaryAbilities[0][exp.ability].number,
...
const PRIMARY_ABILITY_CONFIGS = [
  ...
  {
    mode: 'basic', form: "Altered Human", origin: "Altered Human",
    column: 1, label: "Basic",
    expected: [
  ...
  {
    mode: 'advanced', form: "Altered Human", origin: "Altered Human",
    column: 1, label: "Advanced",
  },

  // Ultimate mode — Normal Human uses column 2 (narrow human ranges)
  {
    mode: 'ultimate', form: "Normal Human", origin: "Natal",
    column: 2, label: "Ultimate",
    expected: [
  ...
  {
    mode: 'ultimate', form: "Robot - Metamorphic", origin: "Robot - Metamorphic",
    column: 4, label: "Robot-Metamorphic",
    rolls: {
      physical:     { Fighting: 10,  Agility: 90,  Strength: 70, Endurance: 50 },
      physicalRow1: { Fighting: 100, Agility: 80,  Strength: 60, Endurance: 40 },
...
Tester._runResourceTests = (gen, mode, physicalForm, origin, column, expectedResources) => {
  ...
  for (const tc of expectedResources) {
    ...
    Tester.assert(charRes.resources.number > 0 && charRes.resources.rank !== "",
      `Resource Roll (${mode}): Resources for ${physicalForm} as '${charRes.resources.rank}' (${charRes.resources.number}).`);
    Tester.assertEquals(tc.rank, charRes.resources.rank,
...
Tester._runUltimateResourceTests = (gen) => {
  ...
  const resourceTests = [
    { form: "Normal Human", origin: "Normal Human", column: 2, cases: [
      { roll: 15, rank: "Good" }, { roll: 50, rank: "Excellent" },
      { roll: 70, rank: "Remarkable" }, { roll: 85, rank: "Incredible" },
      { roll: 95, rank: "Amazing" }, { roll: 100, rank: "Monstrous" },
    ]},
    { form: "Mutant - Random", origin: "Mutant - Random", column: 1, cases: [
      { roll: 15, rank: "Feeble" }, { roll: 50, rank: "Poor" },
      { roll: 70, rank: "Typical" }, { roll: 85, rank: "Good" },
      { roll: 95, rank: "Excellent" }, { roll: 100, rank: "Remarkable" },
    ]},
  ];
...
const POPULARITY_GEN_ADV_CONFIGS = [
  ...
  { form: "Altered Human",  origin: "Altered Human",  expectedBasic: 10, expectedAdvanced: 30 },
];
...
Tester._runPopularityTests_GenAdv = (gen, mode) => {
  ...
  for (const tc of POPULARITY_GEN_ADV_CONFIGS) {
    ...
    const expected = (mode === 'basic') ? tc.expectedBasic : tc.expectedAdvanced;
    Tester.assertEquals(expected, charPop.popularity,
...
const POPULARITY_ULTIMATE_CONFIGS = [
  { form: "Normal Human",           origin: "Normal Human",           column: 1, secret: true,  roll: 10, expected: 0 },
  { form: "Normal Human",           origin: "Normal Human",           column: 1, secret: false, roll: 10, expected: 13 },
  { form: "Android",                origin: "Android",                column: 4, secret: true,  roll: 10, expected: 0 },
  { form: "Android",                origin: "Android",                column: 4, secret: false, roll: 10, expected: 11 },
  { form: "Surgical Composite",     origin: "Surgical Composite",     column: 2, secret: true,  roll: 10, expected: 0 },
  { form: "Surgical Composite",     origin: "Surgical Composite",     column: 2, secret: false, roll: 10, expected: 11 },
  { form: "Demihuman - Chiropteran", origin: "Demihuman - Chiropteran", column: 2, secret: true,  roll: 10, expected: 0 },
  { form: "Demihuman - Chiropteran", origin: "Demihuman - Chiropteran", column: 2, secret: false, roll: 10, expected: 11 },
  { form: "Angel/Demon",            origin: "Angel/Demon", subType: "Angel",  column: 5, secret: true,  roll: 10, expected: 0 },
  { form: "Angel/Demon",            origin: "Angel/Demon", subType: "Angel",  column: 5, secret: false, roll: 10, expected: 15 },
  { form: "Angel/Demon",            origin: "Angel/Demon", subType: "Demon",  column: 5, secret: true,  roll: 10, expected: 0 },
  { form: "Angel/Demon",            origin: "Angel/Demon", subType: "Demon",  column: 5, secret: false, roll: 10, expected: 11 },
];
...
Tester._runUltimatePopularityTests = (gen) => {
  ...
  for (const test of POPULARITY_ULTIMATE_CONFIGS) {
    ...
    Tester.assertEquals(test.expected, charPop.popularity,
...
Tester.SecondaryAbilityTests = (gen) => {
  ...
  Tester._runResourceTests(gen, 'basic', "Altered Human", "Altered Human", 1, [
    { roll: 1,  rank: "Typical" }, { roll: 16, rank: "Typical" },
    { roll: 51, rank: "Typical" }, { roll: 71, rank: "Typical" },
    { roll: 86, rank: "Typical" }, { roll: 96, rank: "Typical" },
  ]);
  ...
  // Advanced
  Tester._runResourceTests(gen, 'advanced', "Altered Human", "Altered Human", 1, [
    { roll: 1,  rank: "Poor" }, { roll: 16, rank: "Typical" },
    { roll: 51, rank: "Good" }, { roll: 71, rank: "Excellent" },
    { roll: 86, rank: "Remarkable" }, { roll: 96, rank: "Incredible" },
  ]);
  Tester._runResourceTests(gen, 'advanced', "Alien", "Alien", 5, [
    { roll: 1,  rank: "Feeble" }, { roll: 16, rank: "Poor" },
    { roll: 51, rank: "Typical" }, { roll: 71, rank: "Good" },
    { roll: 86, rank: "Excellent" }, { roll: 96, rank: "Remarkable" },
  ]);
