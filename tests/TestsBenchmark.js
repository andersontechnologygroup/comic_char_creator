...
Tester.PerformanceBenchmarkTests = () => {
  ...
  for (const mode of modes) {
    ...
    for (let i = 0; i < iterationsPerMode; i++) {
      ...
      Tester.assert(char.physicalForm !== '', `Perf (${mode}): iteration ${i} should have a physicalForm.`);
    }
    ...
    console.log(`  Performance (${mode}): ${iterationsPerMode} characters in ${elapsed}ms (${perChar}ms/char)`);
  }
  ...
  console.log(`  Performance summary: ${totalChars} characters in ${totalTime}ms total.`);
};
