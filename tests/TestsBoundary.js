...
Tester.ErrorPathTests = () => {
  ...
  Tester.assert(rowLow !== null, `Utility.findRow: Roll 1 column 1 should return a row.`);
  Tester.assertEquals("Feeble", rowLow.rank, `Utility.findRow: Roll 1 column 1 = Feeble.`);

  const rowHigh = Utility.findRow(gen, 100, 1);
  Tester.assert(rowHigh !== null, `Utility.findRow: Roll 100 column 1 should return a row.`);
  Tester.assertEquals("Amazing", rowHigh.rank, `Utility.findRow: Roll 100 column 1 = Amazing.`);

  // Roll 0 matches first row (Feeble) since 0 <= 5
  const rowZero = Utility.findRow(gen, 0, 1);
  Tester.assert(rowZero !== null, `Utility.findRow: Roll 0 should return a row (matches first entry).`);
  Tester.assertEquals("Feeble", rowZero.rank, `Utility.findRow: Roll 0 = Feeble.`);
  ...
  Tester.assert(rowBoundary !== null,
    `Utility.findRow: Roll 100 should return a row (matches last entry).`);
  Tester.assertEquals("Amazing", rowBoundary.rank,
  ...
  Tester.assertEquals(0, charZero.health, `calculateSecondary: Zero abilities = zero health.`);
  Tester.assertEquals(0, charZero.karma, `calculateSecondary: Zero abilities = zero karma.`);
}
...
Tester.DiceSeedTests = () => {
  ...
  for (let i = 0; i < 10; i++) {
    Tester.assertEquals(seq1[i], seq2[i],
  ...
  for (let i = 0; i < 20; i++) {
    Tester.assertEquals(1, Dice.roll(1, 1),
      `Dice.roll(1,1): Always returns 1.`);
    Tester.assertEquals(50, Dice.roll(50, 50),
      `Dice.roll(50,50): Always returns 50.`);
    Tester.assertEquals(100, Dice.roll(100, 100),
  ...
  for (let i = 0; i < 20; i++) {
    ...
    Tester.assertInRange(1, 100, r, `Dice.roll(1,100): Result in range [1-100].`);
  }

  // Boundary pair: result stays in [99, 100]
  for (let i = 0; i < 20; i++) {
    const r = Dice.roll(99, 100);
    Tester.assert(r === 99 || r === 100,
  ...
  for (let i = 0; i < 20; i++) {
    const r = Dice.roll(1, 100);
    Tester.assert(Number.isInteger(r),
  ...
  Tester.assertInRange(1, 100, r1, `Dice.roll100 (unseeded): Result in range [1-100].`);
  Tester.assertInRange(1, 100, r2, `Dice.roll100 (unseeded): Result in range [1-100].`);
}
...
Tester.NegativeInputTests = () => {
  ...
  Tester.assert(typeof invertedResult === 'number',
  ...
  Tester.assertEquals(null, emptyResult,
  ...
  Tester.assertEquals(undefined, invalidAbility,
  ...
  Tester.assertEquals(10, char.getAbility("Fighting").number,
  ...
  Tester.assert(genBad.powerCategoriesTable !== null && genBad.powerCategoriesTable !== undefined,
    `setTables("bogus_mode"): powerCategoriesTable is set (uses basic default).`);
  Tester.assert(genBad.talentCategoriesTable !== null && genBad.talentCategoriesTable !== undefined,
  ...
  Tester.assertEquals(10, charPartial.health,
    `calculateSecondary: Partial abilities — health = Fighting only (10).`);
  Tester.assertEquals(0, charPartial.karma,
  ...
  Tester.assert(charMinimal !== null && charMinimal !== undefined,
  ...
  Tester.assert(typeof charMinimal.physicalForm === 'string',
...
Tester.CorruptedTableTests = () => {
  ...
  Tester.assert(char1 !== null, `CorruptedTable: Empty power table doesn't crash.`);
  // Note: powersCount may still be > 0 from quantityTable; the key test is no crash
  Tester.assert(typeof char1.powersCount === 'number',
  ...
  gen2.powerListTable[0] = { maxRoll: 100, category: 'Test', name: undefined, rank: 'Good' };

  const char2 = gen2.generateWithoutThrows();
  Tester.assert(char2 !== null, `CorruptedTable: Missing power name doesn't crash.`);
  ...
  gen3.talentListTable = [{ maxRoll: 100, category: null, name: 'Test Talent', code: 'T1' }];
  gen3.talentCategoriesTable = [{ maxRoll: 100, name: null }];

  const char3 = gen3.generateWithoutThrows();
  Tester.assert(char3 !== null, `CorruptedTable: Null talent category doesn't crash.`);
  ...
  Tester.assert(char4 !== null, `CorruptedTable: Empty contact tables don't crash.`);
  Tester.assert(typeof char4.contactsCount === 'number',
  ...
  Tester.assert(char5 !== null, `CorruptedTable: Negative rankNumber doesn't crash.`);
  ...
  Tester.assert(char6 !== null, `CorruptedTable: Missing 'column' in physical form doesn't crash.`);
  ...
  Tester.assert(charPure.physicalForm !== '',
    `generateCharacter: physicalForm is populated.`);
  Tester.assert(charPure.origin !== '',
    `generateCharacter: origin is populated.`);
  Tester.assert(typeof charPure.health === 'number' && charPure.health >= 0,
    `generateCharacter: health is a non-negative number.`);
  Tester.assert(typeof charPure.karma === 'number' && charPure.karma >= 0,
  ...
  Tester.assert(charPureUlt !== null, `generateCharacter (ultimate): Returns a character.`);
  Tester.assert(charPureUlt.weakness.stimulus !== '',
  ...
  Tester.assert(charClamped !== null, `CorruptedTable: Out-of-range number rolls don't crash.`);
  Tester.assert(typeof charClamped.powersCount === 'number',
    `CorruptedTable: powersCount is still valid after clamp.`);
  Tester.assert(typeof charClamped.talentsCount === 'number',
    `CorruptedTable: talentsCount is still valid after clamp.`);
  Tester.assert(typeof charClamped.contactsCount === 'number',
