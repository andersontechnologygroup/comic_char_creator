...
Tester.GenerateCharacterInvalidOptionsTests = () => {
  ...
  Tester.assert(char1 !== null, "generateCharacter('bogus_mode'): returns a character.");
  Tester.assert(char1.physicalForm !== "", "generateCharacter('bogus_mode'): physicalForm is set.");
  ...
  Tester.assert(char2 !== null, "generateCharacter(seed=null): returns a character.");

  // Undefined options: should use defaults
  const char3 = CharacterGenerator.generateCharacter('basic', undefined);
  Tester.assert(char3 !== null, "generateCharacter(undefined options): returns a character.");

  // Empty options object
  const char4 = CharacterGenerator.generateCharacter('ultimate', {});
  Tester.assert(char4 !== null, "generateCharacter(empty options): returns a character.");
  Tester.assert(char4.weakness.stimulus !== undefined,
  ...
  Tester.assert(char5 !== null, "generateCharacter(physicalFormRoll=-1): returns a character.");
  ...
  Tester.assert(char6 !== null, "generateCharacter(physicalFormRoll=200): returns a character.");
}
...
Tester.DetermineHealthEdgeCaseTests = (gen) => {
  ...
  Tester.assertEquals(40, char1.health,
    "determineHealth: Mineral health *2 → 40.");
  Tester.assertEquals("multiply", char1.state.healthAdjustment.type,
  ...
  Tester.assertEquals(32, char2.health,
    "determineHealth: health /2 → 32.");
  Tester.assertEquals("divide", char2.state.healthAdjustment.type,
  ...
  Tester.assertEquals(25, char3.health,
    "determineHealth: health +5 → 25.");
  Tester.assertEquals("add", char3.state.healthAdjustment.type,
...
Tester.DeterminePopularityBranchTests = (gen) => {
  ...
  Tester.assert(typeof char3.popularity === 'number',
  ...
  Tester.assert(typeof char4.popularity === 'number',
  ...
  Tester.assert(typeof char5.popularity === 'number',
...
Tester.DetermineOriginBranchTests = (gen) => {
  ...
  Tester.assertEquals("", char1.origin,
  ...
  Tester.assertEquals("", char2.origin,
...
Tester.DetermineAbilityBranchTests = (gen) => {
  ...
  const physicalFormRow = gen.physicalFormTable.find(o => o.name === "Robot - Computer");
  const rankRow = gen.determineAbility(char1, "Fighting", physicalFormRow, 0);
  // Robot - Computer has fightingSet defined
  Tester.assert(rankRow !== null || physicalFormRow.fightingSet !== undefined,
  ...
  if (hasStart) {
    ...
    const capName = abilityName.charAt(0).toUpperCase() + abilityName.slice(1);
    const rr = gen.determineAbility(char2, capName, nhRow, 0);
    Tester.assert(rr !== null,
      `determineAbility: Normal Human ${capName} uses *Start path.`);
  } else {
    Tester.assert(true, "determineAbility: Normal Human has no *Start attrs, skip.");
  }
}
...
Tester.GenerateWithSeedTests = () => {
  ...
  for (const mode of ['basic', 'advanced', 'ultimate']) {
    ...
    Tester.assert(char !== null, `generate(${mode}, seed=42): returns a character.`);
    Tester.assert(char.physicalForm !== '', `generate(${mode}, seed=42): physicalForm is set.`);
    Tester.assert(char.origin !== '', `generate(${mode}, seed=42): origin is set.`);
    Tester.assert(typeof char.health === 'number' && char.health >= 0,
      `generate(${mode}, seed=42): health is a non-negative number.`);
    Tester.assert(typeof char.karma === 'number' && char.karma >= 0,
    ...
    if (mode === 'ultimate') {
      Tester.assert(char.weakness.stimulus !== undefined,
  ...
  Tester.assertEquals(char1.physicalForm, char2.physicalForm,
    'generate(): same seed produces same physicalForm.');
  Tester.assertEquals(char1.origin, char2.origin,
    'generate(): same seed produces same origin.');
  Tester.assertEquals(char1.health, char2.health,
    'generate(): same seed produces same health.');
  Tester.assertEquals(char1.powers.length, char2.powers.length,
...
Tester.AbilitySetStartPathTests = (gen) => {
  ...
  if (!row) { Tester.assert(true, 'AbilitySetStart: Altered Human not found, skip.'); return; }
  ...
  Tester.assertEquals('Excellent', char1.getAbility('Fighting').rank,
  ...
  Tester.assertEquals('Good', char2.getAbility('Fighting').rank,
  ...
  Tester.assert(char3.getAbility('Fighting').rank !== '',
