...
Tester.AbilitySetPathTests = () => {
  ...
  if (!form) { Tester.assert(true, 'AbilitySetPath: skipped.'); return; }
  ...
  try {
    ...
    Tester.assertEquals('Excellent', fighting.rank,
...
Tester.AbilityNormalPathTests = () => {
  ...
  Tester.assert(agility.rank !== '' && agility.number > 0,
...
Tester.GetAbilityAfterModifierEdgeTests = () => {
  ...
  const r1 = gen.getAbilityAfterModifier(char, 'Fighting', 'Feeble', 0, 'Typical', -1);
  Tester.assertEquals('Typical', r1, 'ModifierMin: Feeble clamped to Typical.');

  // max clamping (768-770): Monstrous adjusted by 0, max=Amazing → clamped down
  const r2 = gen.getAbilityAfterModifier(char, 'Fighting', 'Monstrous', 0, -1, 'Amazing');
  Tester.assertEquals('Amazing', r2, 'ModifierMax: Monstrous clamped to Amazing.');

  // Shift 0 → Feeble (774-776): adjustment drops rank to Shift 0
  const r3 = gen.getAbilityAfterModifier(char, 'Fighting', 'Shift 0', 0, -1, -1);
  Tester.assertEquals('Feeble', r3, 'ModifierShift0: Shift 0 becomes Feeble.');
};
...
Tester.HiTechToGoodFalseTests = () => {
  ...
  const hiTechForm = gen.physicalFormTable.find(r =>
    Utility.getValue(r, 'isHiTech', false) === true
  );
  if (!hiTechForm) { Tester.assert(true, 'HiTechToGoodFalse: skipped.'); return; }
  ...
  Tester.assert(char.resources.rank !== undefined,
...
Tester.PopularitySetUltimateTests = () => {
  ...
  const form = gen.physicalFormTable.find(r => r.name === 'Normal Human');
  if (!form) { Tester.assert(true, 'PopSetUlt: skipped.'); return; }
  ...
  try {
    ...
    Tester.assertEquals(15, char.popularity,
      'PopSetUlt: popularity set to 15.');
    Tester.assertEquals(15, char.state.popularity.set,
...
Tester.PopularityStartUltimateTests = () => {
  ...
  const form = gen.physicalFormTable.find(r => r.name === 'Robot - human shape');
  if (!form) { Tester.assert(true, 'PopStartUlt: skipped.'); return; }
  ...
  Tester.assert(char.popularity !== undefined,
    `PopStartUlt: popularity = ${char.popularity}.`);
  Tester.assertEquals(0, char.state.popularity.start,
...
Tester.PopularityMinMaxUltimateTests = () => {
  ...
  const form = gen.physicalFormTable.find(r => r.name === 'Normal Human');
  if (!form) { Tester.assert(true, 'PopMinMaxUlt: skipped.'); return; }
  ...
  try {
    ...
    Tester.assert(char.popularity >= 20,
      `PopMinMaxUlt: popularity (${char.popularity}) >= 20.`);
    Tester.assert(char.popularity <= 30,
...
Tester.PopularityStartBasicTests = () => {
  ...
  if (!form) { Tester.assert(true, 'PopStartGen: skipped.'); return; }
...
Tester.WellEstablishedLooksHumanTests = () => {
  ...
  const form = gen.physicalFormTable.find(r => r.name === 'Mutant');
  if (!form) { Tester.assert(true, 'WE LH: skipped.'); return; }
...
Tester.PowersCountMinimumTests = () => {
  ...
  const form = gen.physicalFormTable.find(r => r.name === 'Normal Human');
  if (!form) { Tester.assert(true, 'PowersCountMin: skipped.'); return; }
  const orig = form.powersCountMinimum;
  form.powersCountMinimum = 5;

  try {
    const char = gen.generateWithoutThrows();
    Tester.assert(char.powersCount >= 5,
...
Tester.BonusPowerAnyRetryTests = () => {
  ...
  if (!energyForm) { Tester.assert(true, 'BonusPowerAnyRetry: skipped.'); return; }
...
Tester.BonusPowerHighRollTests = () => {
  ...
  if (!animalForm) { Tester.assert(true, 'BonusPowerHighRoll: skipped.'); return; }
...
Tester.OptionalPowerAnyRetryTests = () => {
  ...
  const powerWithAny = gen.powerListTable.find(r =>
    r.optionalPowers && r.optionalPowers.indexOf('Any') !== -1
  );
  if (!powerWithAny) {
  ...
  const catRow = gen.powerCategoriesTable.find(c => c.name === powerWithAny.category);
  if (!catRow) {
