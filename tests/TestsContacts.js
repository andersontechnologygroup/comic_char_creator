...
Tester._runContactsTooManyTests = (gen, mode, form, column) => {
  ...
  Tester.assertInRange(0, expectedMax, cCount,
    `Contact Quantity Roll (${mode}): Generated ${cCount} contacts.`);
  Tester.assert(cCount >= 3,
  ...
  Tester.assertInRange(0, expectedMax, cCount2,
    `Contact Quantity Roll (${mode}): Generated ${cCount2} contacts.`);
  Tester.assert(cCount2 >= 2,
...
const CONTACT_QUANTITY_CASES = [
  { contactNumberRoll: 74, contactCategoryRolls: [2, 27, 67, 91], contactRolls: [20, 30, 40, 50], expectedCount: 2 },
  { contactNumberRoll: 17, contactCategoryRolls: [2, 27, 67, 91], contactRolls: [20, 30, 40, 50], expectedCount: 0 },
  { contactNumberRoll: 25, contactCategoryRolls: [2, 27, 67, 91], contactRolls: [20, 30, 40, 50], expectedCount: 1 },
  { contactNumberRoll: 69, contactCategoryRolls: [2, 27, 67, 91], contactRolls: [20, 30, 40, 50], expectedCount: 2 },
];
...
Tester.ContactsTests = (gen) => {
  ...
  Tester._runContactQuantityTests(gen, {
    mode: 'basic',
    charSetup: { physicalForm: "Altered Human", origin: "Altered Human", column: 1 },
...
Tester.ContactDuplicatePreventionTests = (gen) => {
  ...
  Tester.assert(gen._assignedContactNames === null,
...
Tester.ContactCountTests = (gen) => {
  ...
  for (const entry of gen.contactTypeListTable) {
    Tester.assertEquals(1, Utility.getValue(entry, 'contactCount', 1),
...
Tester.ContactCountGeneratedTests = (gen) => {
  ...
  const basicContact = gen.contactTypeListTable.find(c => c.name && c.name !== '_____________');
  if (!basicContact) {
    Tester.assert(true, 'ContactCountGenerated(Basic): No real contacts in table, skip.');
  } else {
    const catEntry = gen.contactCategoriesTable.find(c => c.name === basicContact.category);
    if (catEntry) {
      ...
      const char = gen.generateWithoutThrows();
      const found = char.contacts.find(c => c.name === basicContact.name);
      if (found) {
        const expectedCC = Utility.getValue(basicContact, 'contactCount', 1);
        Tester.assertEquals(expectedCC, found.contactSlots,
          `ContactCountGenerated(Basic): ${basicContact.name} contactSlots = ${expectedCC}.`);
      } else {
        Tester.assert(true, `ContactCountGenerated(Basic): ${basicContact.name} not rolled, skip.`);
      }
    }
  }
  ...
  const medCatAdv = gen.contactCategoriesTable.find(c => c.name === medAdv.category);
  ...
  if (medC) {
    Tester.assertEquals(1, medC.contactSlots,
      'ContactCountGenerated(Advanced): Medicine contactSlots = 1 (default).');
  } else {
    Tester.assert(true, 'ContactCountGenerated(Advanced): Medicine not rolled, skip.');
  }
  ...
  const medCatUlt = gen.contactCategoriesTable.find(c => c.name === medUlt.category);
  ...
  if (medU) {
    Tester.assertEquals(1, medU.contactSlots,
      'ContactCountGenerated(Ultimate): Medicine contactSlots = 1 (default).');
  } else {
    Tester.assert(true, 'ContactCountGenerated(Ultimate): Medicine not rolled, skip.');
  }
};
...
Tester.ContactCountDefaultTests = (gen) => {
  ...
  const lawCatAdv = gen.contactCategoriesTable.find(c => c.name === lawAdv.category);
  ...
  if (lawC) {
    Tester.assertEquals(1, lawC.contactSlots,
      'ContactCountDefault(Advanced): Law contactSlots = 1 (default).');
  } else {
    Tester.assert(true, 'ContactCountDefault(Advanced): Law not rolled, skip.');
  }
  ...
  const lawCatUlt = gen.contactCategoriesTable.find(c => c.name === lawUlt.category);
  ...
  if (lawU) {
    Tester.assertEquals(1, lawU.contactSlots,
      'ContactCountDefault(Ultimate): Law contactSlots = 1 (default).');
  } else {
    Tester.assert(true, 'ContactCountDefault(Ultimate): Law not rolled, skip.');
  }
};
...
Tester.ContactCountSlotSkipTests = (gen) => {
  ...
  const medEntry = gen.contactTypeListTable.find(c => c.name === 'Medicine');
  if (!medEntry) {
    Tester.assert(true, 'ContactCountSlotSkip: Medicine not found in table, skip.');
    return;
  }
  ...
  const medCat = gen.contactCategoriesTable.find(c => c.name === medEntry.category);
