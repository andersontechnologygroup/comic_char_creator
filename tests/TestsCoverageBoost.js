...
Tester.BoostFunctionTests = () => {
  ...
  Tester.assert(newFighting !== oldFighting || oldFighting === 'Amazing',
  ...
  Tester.assert(result2 === false, 'boostAbility: fails when boostApplied=true.');
  ...
  if (char.powers.length > 0) {
    ...
    Tester.assert(powerResult === true, 'boostPower: returns true on success.');
    Tester.assert(char.powers[0].rank !== oldRank || oldRank === 'Amazing',
...
Tester.PopularitySubTypeRulesTests = () => {
  ...
  if (angelChar.physicalForm === 'Angel/Demon') {
    ...
    if (angelChar.subType === 'Angel') {
      Tester.assertEquals(2, angelChar.state.popularity.adjustment,
  ...
  if (row) {
    ...
    Tester.assert(typeof char.popularity === 'number',
...
Tester.DeterminePopularityBasicOriginIdentityTests = () => {
  ...
  Tester.assert(char4.popularity >= 0,
...
Tester.DeterminePopularityMinMaxClampTests = () => {
  ...
  const formsWithMin = gen.physicalFormTable.filter(r =>
    typeof r.popularityMinimum === 'number' && r.popularityMinimum > 0);
  const formsWithMax = gen.physicalFormTable.filter(r =>
    typeof r.popularityMaximum === 'number');
  ...
  if (formsWithMin.length > 0) {
    ...
    Tester.assert(char.popularity >= form.popularityMinimum,
  ...
  if (formsWithMax.length > 0) {
    ...
    gen.looksHuman = true;
    gen.determinePopularity(char);
    Tester.assert(char.popularity <= form.popularityMaximum,
...
Tester.ContactsEqualToPowersTests = () => {
  ...
  Tester.assertEquals(char.powersCount, char.contactsCount,
    'contactsEqualToPowers: contacts = powers count.');
  Tester.assert(gen.contactsEqualToPowers === true,
...
Tester.ContactTalentCountMinMaxTests = () => {
  ...
  if (!row) { Tester.assert(true, 'ContactTalentCountMinMaxTests: Altered Human not found, skip.'); return; }
  ...
  char2.physicalForm = 'Altered Human';
  gen.determineSpecialAbilities(char2);
  Tester.assert(char2.contactsCount <= 0,
  ...
  Tester.assert(char3.talents.length >= 0,
  ...
  Tester.assert(char4.talentsCount <= 0,
...
Tester.BonusPowerOfPhysicalFormTests = () => {
  ...
  if (char.physicalForm === 'Angel/Demon') {
    Tester.assert(typeof char.subType === 'string',
    ...
    Tester.assert(bonusPowers.length >= 0,
  ...
  const formsWithAny = gen.physicalFormTable.filter(r =>
    r.bonusPower && r.bonusPower.indexOf('Any') !== -1);
  ...
  if (formsWithAny.length > 0) {
    ...
    gen.determineSpecialAbilities(char2);
    Tester.assert(char2.powers.length >= 0,
...
Tester.GenerateOptionalPowerTests = () => {
  ...
  const formWithOptional = gen.physicalFormTable.find(r =>
    r.optionalPowers && r.optionalPowers.length > 0);
  ...
  if (formWithOptional) {
    ...
    char.powers.push({
      name: 'Existing Power', code: 'XX', category: 'Test',
  ...
  const formWithTilde = gen.physicalFormTable.find(r =>
    r.optionalPowers && r.optionalPowers.indexOf('~') !== -1);
  ...
  if (formWithTilde) {
    ...
    gen.powerRankRolls = Array(gen.rollArraySize).fill(50);
    gen.determineSpecialAbilities(char);
    Tester.assert(char.powers.length >= 0,
...
Tester.GenerateBonusPowerFromAbilityTests = () => {
  ...
  const powerRow = gen.powerListTable.find(p =>
    typeof p.bonusPowerCount === 'number' && p.bonusPowerCount > 0 && p.bonusPower);
...
Tester.GenerateSinglePowerDuplicateTests = () => {
  ...
  if (char.powers.length > 0) {
    ...
    char.powers.push({
      name: firstName, code: 'XX', category: 'Test',
      description: '', rank: 'Typical', number: 6, powerSlots: 1
    });
...
Tester.GenerateTalentsReRollTests = () => {
  ...
  Tester.assert(char.talents.length >= 0,
...
Tester.ContactGenerationBranchTests = () => {
  ...
  gen.determineSpecialAbilities(char);
  Tester.assert(char.contacts.length >= 0,
...
Tester.PopularityUltimateStartPathTests = () => {
  ...
  const formWithStart = gen.physicalFormTable.find(r =>
    typeof r.popularityStart === 'number' && r.popularityStart > 0);
  ...
  if (formWithStart) {
    ...
    gen.popularityRoll = 50;
    gen.determinePopularityUltimate(char);
    Tester.assert(typeof char.popularity === 'number' && char.popularity >= 0,
...
Tester.HealthAdjustmentBranchTests = () => {
  ...
  const row = gen.physicalFormTable.find(o => o.name === 'Altered Human');
  if (!row) { Tester.assert(true, 'HealthAdj: Altered Human not found, skip.'); return; }
  ...
  Tester.assert(char1.state.healthAdjustment && char1.state.healthAdjustment.type === 'add',
  ...
  Tester.assert(char3.state.healthAdjustment && char3.state.healthAdjustment.type === 'divide',
...
Tester.AbilityAdjustmentBranchTests = () => {
  ...
  const mutantRow = gen.physicalFormTable.find(o => o.name === 'Mutant - Induced');
  if (mutantRow) {
    // Temporarily add anyPrimaryAbilityAdjustment to a basic form
    const geng = new CharacterGenerator();
    geng.generatorMode = 'basic';
    geng.setTables();
    const testRow = geng.physicalFormTable.find(o => o.name === 'Altered Human');
    if (testRow) {
      ...
      Tester.assert(char.state.anyAbilityAdjustment !== undefined,
        'AbilityAdj: anyPrimaryAbilityAdjustment applied.');
      if (origVal !== undefined) testRow.anyPrimaryAbilityAdjustment = origVal;
  ...
  // --- allPrimaryAbilitiesAdjustment (line 693-708) ---
  {
    ...
    if (testRow) {
      ...
      char.calculateSecondary();
      geng.determinePrimaryAbilities(char);
      Tester.assert(char.state.allPrimaryAbilityAdjustment !== undefined,
        'AbilityAdj: allPrimaryAbilitiesAdjustment applied.');
      Tester.assert(char.state.allPrimaryAbilityAdjustment.adjustment === -1,
        'AbilityAdj: allPrimary adjustment = -1.');
      if (origVal !== undefined) testRow.allPrimaryAbilitiesAdjustment = origVal;
  ...
  // --- allPhysicalAbilitiesAdjustment (line 710-725) ---
  {
    ...
    if (testRow) {
      ...
      char.calculateSecondary();
      geng.determinePrimaryAbilities(char);
      Tester.assert(char.state.allPhysicalAbilityAdjustment !== undefined,
        'AbilityAdj: allPhysicalAbilitiesAdjustment applied.');
      if (origVal !== undefined) testRow.allPhysicalAbilitiesAdjustment = origVal;
...
Tester.CrossCategoryPowerShiftTests = () => {
  ...
  if (minCat) {
    ...
    for (const p of catPowers) {
      char.powers.push({
        ...
        description: p.description || '', rank: 'Typical', number: 6, powerSlots: 1
      });
    }
    ...
    if (catEntry) {
      ...
      Tester.assert(char.powers.length >= catPowers.length,
  ...
  // Also test: undefined powerRow (roll doesn't match any power) with cross-category fallback
  {
    ...
    if (minCat) {
      const catPowers = gen2.powerListTable.filter(p => p.category === minCat);
      for (const p of catPowers) {
        char2.powers.push({
          name: p.name, code: p.code, category: p.category,
          description: p.description || '', rank: 'Typical', number: 6, powerSlots: 1
      ...
      char2._assignedPowerNames = new Set(char2.powers.map(p => p.name));

      const catEntry = gen2.powerCategoriesTable.find(c => c.name === minCat);
      if (catEntry) {
        const targetRoll = Math.min(catEntry.maxRoll, 50);
        gen2.powerCategoryRolls = Array(gen2.rollArraySize).fill(targetRoll);
        ...
        Tester.assert(char2.powers.length >= catPowers.length,
...
Tester.DiceClassTests = () => {
  ...
  Tester.assert(Dice._rngState === null, 'Dice.seed(null): rngState cleared.');
  ...
  Tester.assert(r100 >= 1 && r100 <= 100, `Dice.roll100: ${r100} in [1,100].`);
  ...
  for (let i = 0; i < 50; i++) {
    ...
    Tester.assert(items.includes(item), `Dice.getRandomItem: "${item}" is in array.`);
  }
  ...
  Tester.assertEquals('only', result, 'Dice.getRandomItem: single-element → always that element.');
...
Tester.PopularitySubTypeMinMaxTests = () => {
  ...
  const row = gen.physicalFormTable.find(o => o.name === 'Angel/Demon');
  if (!row) { Tester.assert(true, 'PopSubTypeMinMax: Angel/Demon not found, skip.'); return; }
  ...
  Tester.assert(char1.popularity >= 5 && char1.popularity <= 50,
  ...
  gen.popularityRoll = 50;
  gen.determinePopularityUltimate(char2);
  Tester.assert(char2.popularity >= 0 && char2.popularity <= 30,
    `PopSubTypeMinMax: Demon popularity=${char2.popularity} in [0,30].`);

  // Restore
  if (origMin !== undefined) row.popularityMinimum = origMin; else delete row.popularityMinimum;
  if (origMax !== undefined) row.popularityMaximum = origMax; else delete row.popularityMaximum;
}
...
Tester.PopularityWellEstablishedLooksHumanTests = () => {
  ...
  gen.looksHuman = true;
  gen.determinePopularity(char3);
  Tester.assert(char3.popularity > char1.popularity || char3.popularity > char2.popularity,
...
Tester.SpecialAbilitySetAdjustmentTests = () => {
  ...
  const row = gen.physicalFormTable.find(o => o.name === 'Altered Human');
  if (!row) { Tester.assert(true, 'SpecialAbilitySetAdj: Altered Human not found, skip.'); return; }

  // Save originals
  const saved = {};
  ['contactsCountSet', 'contactsCountAdjustment', 'talentsCountSet', 'talentsCountAdjustment',
   'powersCountSet', 'powersCountAdjustment', 'powersCountMinimum', 'powersCountMaximum'
  ].forEach(k => { saved[k] = row[k]; });
  ...
  char1.physicalForm = 'Altered Human';
  gen.determineSpecialAbilities(char1);
  Tester.assertEquals(7, char1.contactsCount, 'SpecialAdj: contactsCountSet=7 → 7.');
  char1 = null;
  ...
  gen.determineSpecialAbilities(char1);
  Tester.assert(char1.contactsCount >= origCC,
  ...
  char1.physicalForm = 'Altered Human';
  gen.determineSpecialAbilities(char1);
  Tester.assertEquals(5, char1.talentsCount, 'SpecialAdj: talentsCountSet=5 → 5.');
  char1 = null;
  ...
  gen.determineSpecialAbilities(char1);
  Tester.assert(char1.talentsCount >= origTC,
  ...
  char1.physicalForm = 'Altered Human';
  gen.determineSpecialAbilities(char1);
  Tester.assertEquals(8, char1.powersCount, 'SpecialAdj: powersCountSet=8 → 8.');
  char1 = null;
  ...
  gen.determineSpecialAbilities(char1);
  Tester.assert(char1.powersCount >= origPC,
  ...
  char1.physicalForm = 'Altered Human';
  gen.determineSpecialAbilities(char1);
  Tester.assert(char1.powersCount >= 99 || char1.powersCount >= 1,
  ...
  char1.physicalForm = 'Altered Human';
  gen.determineSpecialAbilities(char1);
  Tester.assert(char1.powersCount <= 0,
...
Tester.InvalidCategoryAndPowerRollTests = () => {
  ...
  Tester.assert(char3.powers.length >= 0,
...
Tester.BonusPowerAnyPathTests = () => {
  ...
  if (!deityRow) { Tester.assert(true, 'BonusPowerAny: Deity not found, skip.'); return; }
  ...
  if (char.powers.length > 0) {
    Tester.assertEquals('Travel', char.powers[0].category,
...
Tester.OptionalPowerAnyPathTests = () => {
  ...
  gen.generateOptionalPower(char2, 5, 'Energy Control\\Thermal Control|Energy Emission\\Any');
  Tester.assert(char2.powers.length >= 0,
  ...
  gen.generateOptionalPower(char3, 5, 'Magic\\Any|Detection\\Extradimensional');
  Tester.assert(char3.powers.length >= 0,
...
Tester.DiceEdgeCaseTests = () => {
  ...
  Tester.assert(undefResult === null,
  ...
  Tester.assert(emptyResult === null,
  ...
  Tester.assert(['x', 'y', 'z'].includes(validResult),
