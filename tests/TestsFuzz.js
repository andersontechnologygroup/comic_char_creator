...
Tester._assertPowerInvariants = (char, mode) => {
    char.powers.forEach((p, i) => {
        Tester.assert(
            p.name !== "" && p.name !== undefined && p.name !== null,
        ...
        Tester.assert(
            p.rank !== "" && p.rank !== undefined && p.rank !== null,
            `Fuzz (${mode}): power[${i}].rank is non-empty (was "${p.rank}").`,
        );
        Tester.assert(
            typeof p.powerSlots === "number" && p.powerSlots >= 0,
...
Tester._assertTalentInvariants = (char, mode) => {
    char.talents.forEach((t, i) => {
        Tester.assert(
            t.name !== "" && t.name !== undefined && t.name !== null,
        ...
        Tester.assert(
            typeof t.talentSlots === "number" && t.talentSlots >= 1,
...
Tester._assertContactInvariants = (char, mode) => {
    char.contacts.forEach((c, i) => {
        Tester.assert(
            c.name !== "" && c.name !== undefined && c.name !== null,
...
Tester._assertResourceInvariants = (char, mode) => {
    ...
    Tester.assert(
        validRanks.indexOf(char.resources.rank) !== -1,
...
Tester._assertIdentityInvariants = (char, mode) => {
    Tester.assert(
        char.identity === "Secret" || char.identity === "Public",
...
Tester._assertCharactorInvariants = (char, mode) => {
    ...
    Tester.assert(
        typeof char.health === "number",
        `Fuzz (${mode}): health is a number.`,
    );
    Tester.assertNotNull(
        char.resources,
        `Fuzz (${mode}): resources object exists.`,
    );
    ...
    Tester.assert(char.origin !== "", `Fuzz (${mode}): origin is non-empty.`);
    ...
    if (mode === "ultimate") {
        ...
        Tester.assert(
            typeof char.weakness.rankNumber === "number" &&
                char.weakness.rankNumber > 0,
...
Tester.FuzzDiceTests = () => {
    ...
    for (let i = 0; i < 100; i++) {
        ...
        Tester.assertInRange(
            ...
            100,
            result,
            `Dice.roll100(): result in range [1-100].`,
        );
    }
...
Tester.FuzzPhysicalFormDiversityTests = (gen) => {
    ...
    gen.generatorMode = ["basic", "advanced", "ultimate"][
        Tester._fuzzFormSeed % 3
    ];
    gen.setTables();
    gen.throwAllRolls();
