...
Tester._hasLogEntry = function(log, description, secondTerm) {
  ...
  return log.some(function(l) {
    ...
    const isWordChar = function(c) { return /[a-zA-Z0-9]/.test(c); };
...
Tester._hasLogContaining = function(log, substring) {
  return log.some(function(l) { return l.indexOf(substring) !== -1; });
};
...
Tester._assertNoGarbageInLogs = function(log, mode) {
  ...
  Tester.assert(!Tester._hasLogContaining(log, 'undefined'),
    prefix + "logs don't contain 'undefined'.");
  Tester.assert(!Tester._hasLogContaining(log, 'null'),
    prefix + "logs don't contain 'null'.");
  Tester.assert(!Tester._hasLogContaining(log, '[object Object]'),
...
Tester.HelperFunctionTests = () => {
  // --- _hasLogEntry ---
  const log1 = ['[Test] entry1', 'Ability: Fighting Adjustment: -1', 'Simple message'];

  // Bracket format match
  Tester.assert(Tester._hasLogEntry(log1, 'Test'),
    '_hasLogEntry: bracket format matches.');
  Tester.assert(Tester._hasLogEntry(log1, 'Test', ''),
    '_hasLogEntry: bracket format with empty secondTerm matches.');
  Tester.assert(!Tester._hasLogEntry(log1, 'Nonexistent'),
    '_hasLogEntry: non-existent description returns false.');

  // Word-boundary matching
  Tester.assert(Tester._hasLogEntry(log1, 'Ability', 'Fighting'),
    '_hasLogEntry: word-boundary match for "Fighting".');
  Tester.assert(Tester._hasLogEntry(log1, 'Adjustment', '-1'),
    '_hasLogEntry: word-boundary match for "-1".');
  Tester.assert(!Tester._hasLogEntry(log1, 'Adjustment', '-'),
    '_hasLogEntry: partial match "-" should not match "-1".');

  // null/undefined secondTerm treated as empty
  Tester.assert(!Tester._hasLogEntry(log1, 'Ability', null),
    '_hasLogEntry: null secondTerm returns false (description not in brackets).');
  Tester.assert(!Tester._hasLogEntry(log1, 'Ability', undefined),
  ...
  Tester.assert(!Tester._hasLogEntry(log3, 'ABCX at start', 'ABC'),
  ...
  Tester.assert(!Tester._hasLogEntry(log4, '1ABC test', 'ABC'),
    '_hasLogEntry: secondTerm adjacent to digit at start → no match.');

  // --- _hasLogContaining ---
  Tester.assert(Tester._hasLogContaining(log1, 'Fighting'),
    '_hasLogContaining: substring match.');
  Tester.assert(!Tester._hasLogContaining(log1, 'XYZZY'),
    '_hasLogContaining: non-existent substring returns false.');
  Tester.assert(Tester._hasLogContaining(log1, ''),
