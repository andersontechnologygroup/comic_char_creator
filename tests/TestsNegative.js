...
Tester.GenInvalidModeTests = () => {
    ...
    Tester.assert(
        char1.physicalForm !== "",
        "InvalidMode: physicalForm is set.",
    );
...
Tester.GenNullOptionsTests = () => {
    ...
    Tester.assert(
        typeof char1.physicalForm === "string",
...
Tester.GenExtremeRollTests = () => {
    ...
    Tester.assert(
        char6 !== null,
        "ExtremeOriginRoll(101): returns a character.",
    );
    ...
    Tester.assert(
        char8 !== null,
        "ExtremePowerRoll(100): returns a character.",
    );
    Tester.assert(
        char8.powers.length > 0,
        "ExtremePowerRoll(100): has powers.",
    );
...
Tester.GenResetTests = () => {
    ...
    Tester.assertEquals(
        0,
        gen.physicalFormRoll,
        "Reset: physicalFormRoll cleared to 0.",
    );
    Tester.assertEquals(0, gen.originRoll, "Reset: originRoll cleared to 0.");
};
...
Tester.CharactorSetAbilityTests = () => {
    ...
    Tester.assertEquals(
        "Shift 0",
        char.getAbility("Strength").rank,
        "setAbility: Shift 0 rank accepted.",
    );
    Tester.assertEquals(
        0,
        char.getAbility("Strength").number,
    ...
    Tester.assertEquals(
        "Feeble",
        char.getAbility("Endurance").rank,
    ...
    Tester.assertEquals(
        "Amazing",
        char.getAbility("Reason").rank,
        "setAbility: very high rankNumber accepted.",
    );
    Tester.assertEquals(
        9999,
        char.getAbility("Reason").number,
    ...
    Tester.assertEquals(
        "Typical",
        char.getAbility("Intuition", 0).rank,
        "setAbility: index 0 set.",
    );
    Tester.assertEquals(
        "Good",
        char.getAbility("Intuition", 1).rank,
        "setAbility: index 1 set.",
    );
    Tester.assertEquals(
        "Excellent",
        char.getAbility("Intuition", 2).rank,
    ...
    Tester.assert(
        char.primaryAbilities[50] !== undefined,
        "setAbility: auto-expanded to index 50.",
    );
    Tester.assertEquals(
        "Amazing",
        char.primaryAbilities[50]["Fighting"].rank,
...
Tester.UtilityGetValueEdgeCases = () => {
    ...
    Tester.assertEquals(
        "",
        Utility.getValue({ x: "" }, "x", -1),
        "getValue(empty): returns empty string.",
    );

    // Property is NaN
    Tester.assert(
        isNaN(Utility.getValue({ x: NaN }, "x", -1)),
    ...
    Tester.assertEquals(
        Infinity,
        Utility.getValue({ x: Infinity }, "x", -1),
...
Tester.UtilityFindRowEdgeCases = () => {
    ...
    Tester.assert(
        rowRollNeg !== null,
        "findRow(roll=-1): returns a row (matches first entry).",
    );
...
Tester.DiceEdgeCasesStressTests = () => {
    ...
    Tester.assertEquals(
        "only",
        single,
        "Dice.getRandomItem(single): returns element.",
    );
...
Tester.PowerGenerationFailureTests = () => {
    ...
    if (minCat) {
        ...
        if (catEntry) {
            gen.powerCategoryRolls = Array(gen.rollArraySize).fill(
                Math.min(catEntry.maxRoll, 50),
            );
            gen.powerRolls = Array(gen.rollArraySize).fill(50);
            gen.powerRankRolls = Array(gen.rollArraySize).fill(50);
            gen.generateSinglePower(char5, 0);
            Tester.assert(
                char5.powers.length >= catPowers.length,
...
Tester.HealthAdjustmentEdgeCaseTests = () => {
    ...
    Tester.assertEquals(
        -990,
        char3.health,
        "HealthEdge(-1000): large subtract.",
    );
...
Tester.DataTableEdgeCaseTests = () => {
    ...
    gen.powerCategoriesTable.forEach((c) => {
        Tester.assert(
            typeof c.maxRoll === "number" && c.maxRoll > 0,
    ...
    gen.originTable.forEach((o) => {
        Tester.assert(
            typeof o.maxRoll === "number" && o.maxRoll > 0,
...
Tester.CompoundFormEdgeCaseTests = () => {
    ...
    Tester.assert(
        char1 !== null,
        "CompoundEdge(allFirst): returns a character.",
    );
    Tester.assertEquals(
        "Compound",
        char1.physicalForm,
    ...
    Tester.assert(
        char2 !== null,
        "CompoundEdge(allLast): returns a character.",
    );
    Tester.assertEquals(
        "Compound",
        char2.physicalForm,
