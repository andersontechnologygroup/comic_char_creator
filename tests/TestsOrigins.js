...
const ORIGIN_BASIC_CASES = [
    ...
    },
    {
        ...
        physicalFormRoll: 46,
        originRoll: 76,
        expectedOrigin: "Hi-Tech",
        expectedPhysicalForm: "Altered Human",
        desc: "Altered Human roll=76",
    },
    {
        mode: "basic",
        physicalForm: "Altered Human",
...
const ORIGIN_ULTIMATE_CASES = [
    // Normal Human: physicalFormRoll=1 → "Normal Human", originRoll=5 → "Natal"
    {
        ...
        physicalFormRoll: 1,
        originRoll: 5,
        expectedPhysicalForm: "Normal Human",
        expectedOrigin: "Natal",
        desc: "Normal Human → Natal",
    },
    // Mutant - Random: physicalFormRoll=31, originRoll=15 → "Maturity"
    {
        ...
        physicalFormRoll: 31,
        originRoll: 15,
        expectedPhysicalForm: "Mutant - Random",
        expectedOrigin: "Maturity",
        desc: "Mutant - Random → Maturity",
    },
    // Verify weakness stimulus is set for full-pipeline generation
    {
        ...
        postValidate: (char) => {
            Tester.assert(
                char.state.weakness.stimulus !== null,
...
Tester.OriginUltimateTests = (gen) => {
    ...
    for (const tc of ORIGIN_ULTIMATE_CASES) {
        ...
        gen.physicalFormRoll = tc.physicalFormRoll;
        gen.originRoll = tc.originRoll;
        if (tc.weaknessStimulusRoll !== undefined)
            gen.weaknessStimulusRoll = tc.weaknessStimulusRoll;
        const char = gen.generateWithoutThrows();
        Tester.assertEquals(
            tc.expectedPhysicalForm,
            char.physicalForm,
...
Tester.GeneratorTests = (gen) => {
    ...
    Tester.assert(
        char.physicalForm !== "",
        `Generate: Generated a physical form '${char.physicalForm}'.`,
    );
    Tester.assert(
        char.origin !== "",
        `Generate: Generated an origin '${char.origin}'.`,
    );

    // Abilities should all have values
    PHYSICAL_ABILITIES.forEach((a) => {
        Tester.assert(
            char.getAbility(a).rank !== "",
    ...
    Tester.assertAtLeast(
        1,
        char.powersCount,
        `Generate: Powers count is ${char.powersCount}.`,
    );
    Tester.assertAtLeast(
        0,
        char.talentsCount,
        `Generate: Talents count is ${char.talentsCount}.`,
    );
    Tester.assertAtLeast(
        0,
        char.contactsCount,
        `Generate: Contacts count is ${char.contactsCount}.`,
    );

    // Check physical abilities count
    Tester.assertAtLeast(
        1,
        char.primaryAbilities.length,
