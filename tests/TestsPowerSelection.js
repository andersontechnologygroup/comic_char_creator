...
Tester.PreSelectedPowersBasicTests = (gen) => {
    ...
    for (let i = 0; i < Math.min(char.powers.length, preSelected.length); i++) {
        Tester.assertEquals(
            preSelected[i].powerName,
            char.powers[i].name,
            `PreSelectedPowers: Slot ${i} name = ${char.powers[i].name}.`,
        );
        Tester.assert(
            char.powers[i].rank !== undefined && char.powers[i].rank !== null,
            `PreSelectedPowers: Slot ${i} rank = ${char.powers[i].rank} (rolled).`,
        );
    }

    const chooseLog = char.log.filter(
        (l) => typeof l === "string" && l.indexOf("Choose:") !== -1,
    );
    Tester.assert(
        chooseLog.length >= 1,
...
Tester.PreSelectedPowersFallbackTests = (gen) => {
    ...
    for (let i = 0; i < 10; i++) {
        gen._preSelectedPowers.push({
            category: "Movement",
            powerName: "Nonexistent Power " + i,
        });
    }
    ...
    const fallbackLog = char.log.filter(
        (l) =>
            typeof l === "string" &&
            l.indexOf("Pre-selected power not found") !== -1,
    );
    Tester.assert(
        fallbackLog.length >= 1,
...
Tester.GetPowerSlotsAndCategoriesTests = (gen) => {
    ...
    Tester.assert(
        result.categories.length === result.count,
    ...
    for (let i = 0; i < result.categories.length; i++) {
        Tester.assert(
            validCategories.indexOf(result.categories[i]) !== -1,
...
Tester.BonusPowerSlotReductionTests = (gen) => {
    ...
    Tester.assert(
        hiTechRow !== undefined,
        "BonusSlotReduction: Hi-Tech form found.",
    );
    Tester.assert(
        hiTechRow.bonusPowerCount === 1,
        "BonusSlotReduction: Hi-Tech bonusPowerCount = 1.",
    );
    ...
    const expectedSlotCount = Math.max(
        0,
        basePowersCount - hiTechRow.bonusPowerCount,
    );
    Tester.assertEquals(
        expectedSlotCount,
        result.count,
