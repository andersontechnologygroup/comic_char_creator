...
Tester.SinglePowerBasicTests = (gen) => {
    ...
    Tester.assertEquals(
        1,
        char.powers.length,
        `Power Quantity Roll: Generated ${char.powers.length} powers.`,
    );
    Tester.assertEquals(
        "Resistances and Senses",
        char.powers[0].category,
        `Power Category Roll: Generated ${char.powers[0].category} category.`,
    );
    Tester.assertEquals(
        "Resistance to Cold",
        char.powers[0].name,
        `Power Roll: Generated ${char.powers[0].name} name.`,
    );
    // Column 1: roll 74 → Remarkable (74 > 60 Excellent, ≤ 80 Remarkable)
    Tester.assertEquals(
        "Remarkable",
        char.powers[0].rank,
        `Power Rank Roll: Generated ${char.powers[0].rank} rank.`,
    );
    ...
    Tester.assertEquals(
        "Energy Control",
        char.powers[1].category,
        `Power Category Roll: Generated ${char.powers[1].category} category.`,
    );
    Tester.assertEquals(
        "Electrical Control",
        char.powers[1].name,
        `Power Roll: Generated ${char.powers[1].name} name.`,
    );
    // Column 1: roll 19 → Typical (19 > 10 Poor, ≤ 20 Typical)
    Tester.assertEquals(
        "Typical",
        char.powers[1].rank,
        `Power Rank Roll: Generated ${char.powers[1].rank} rank.`,
    );
    ...
    Tester.assertEquals(
        "Nature Control",
        char.powers[2].category,
        `Power Category Roll: Generated ${char.powers[2].category} category.`,
    );
    Tester.assertEquals(
        "Air Control",
        char.powers[2].name,
        `Power Roll: Generated ${char.powers[2].name} name.`,
    );
    Tester.assertEquals(
        "Good",
        char.powers[2].rank,
        `Power Rank Roll: Generated ${char.powers[2].rank} rank.`,
    );
};
...
Tester.SinglePowerUltimateTests = (gen) => {
    ...
    Tester.assertEquals(
        1,
        char2.powers.length,
        `Power Quantity Roll: Generated ${char2.powers.length} powers.`,
    );
    Tester.assertEquals(
        "Defensive",
        char2.powers[0].category,
        `Power Category Roll: Generated ${char2.powers[0].category} category.`,
    );
    Tester.assertEquals(
        "Force Field",
        char2.powers[0].name,
        `Power Roll: Generated ${char2.powers[0].name} name.`,
    );
    Tester.assertEquals(
        "Remarkable",
        char2.powers[0].rank,
        `Power Rank Roll: Generated ${char2.powers[0].rank} rank.`,
    );
    ...
    Tester.assertEquals(
        "Fighting",
        char2.powers[1].category,
        `Power Category Roll: Generated ${char2.powers[1].category} category.`,
    );
    Tester.assertEquals(
        "Martial Supremacy",
        char2.powers[1].name,
        `Power Roll: Generated ${char2.powers[1].name} name.`,
    );
    Tester.assertEquals(
        "Typical",
        char2.powers[1].rank,
        `Power Rank Roll: Generated ${char2.powers[1].rank} rank.`,
    );
    ...
    Tester.assertEquals(
        "Energy Emission",
        char2.powers[2].category,
        `Power Category Roll: Generated ${char2.powers[2].category} category.`,
    );
    Tester.assertEquals(
        "Heat",
        char2.powers[2].name,
        `Power Roll: Generated ${char2.powers[2].name} name.`,
    );
    Tester.assertEquals(
        "Typical",
        char2.powers[2].rank,
    ...
    Tester.assertEquals(
        1,
        char3.powers.length,
        `Power Quantity Roll: Generated ${char3.powers.length} powers.`,
    );
    Tester.assertEquals(
        "Self-Alteration",
        char3.powers[0].category,
        `Power Category Roll: Generated ${char3.powers[0].category} category.`,
    );
    Tester.assertEquals(
        "Invisibility",
        char3.powers[0].name,
        `Power Roll: Generated ${char3.powers[0].name} name.`,
    );
    Tester.assertEquals(
        "Incredible",
        char3.powers[0].rank,
        `Power Rank Roll: Generated ${char3.powers[0].rank} rank.`,
    );
    Tester.assert(
        char3.powers[0].extraInformation !== "",
    ...
    gen.powerRankRolls = [74, 19, 27, 2];
    gen.powersExtraInfoRolls = [
        100, 8, 20, 40, 52, 60, 68, 76, 84, 8, 20, 40, 52, 60, 68, 76, 84,
    ];
    gen.peiIndex = 0;
    ...
    Tester.assertEquals(
        1,
        char4.powers.length,
        `Power Quantity Roll: Generated ${char4.powers.length} powers.`,
    );
    Tester.assertEquals(
        "Magic",
        char4.powers[0].category,
        `Power Category Roll: Generated ${char4.powers[0].category} category.`,
    );
    Tester.assertEquals(
        "Enchantment",
        char4.powers[0].name,
        `Power Roll: Generated ${char4.powers[0].name} name.`,
    );
    Tester.assertEquals(
        "Ritual/This is a combination of the following Mechanisms into a compound Mechanism. RITUALS (2): Chant/The hero needs to recite a series of words to manifest his Power. The Chant can be any length and form. Gesture/The hero must perform a specific physical action to bring about the desired Effect.",
        char4.powers[0].extraInformation,
...
Tester._runPowerLoopTests = (gen, config) => {
    ...
    for (const targetPower of gen.powerListTable) {
        ...
        const targetIndex = gen.powerListTable.findIndex(
            (p) => p.name === targetPower.name,
        );
        const catData = gen.powerCategoriesTable.find(
            (c) => c.name == targetPower.category,
        );

        let indexAdjustment = 1;
        let powerRow1 =
            targetIndex <= 4
                ? gen.powerListTable[targetIndex + 1]
                : gen.powerListTable[gen.powerListTable.length - 4];
        while (powerRow1.maxRoll > 100) {
            powerRow1 =
                targetIndex <= 4
                    ? gen.powerListTable[targetIndex + indexAdjustment]
                    : gen.powerListTable[
                          gen.powerListTable.length - indexAdjustment
                      ];
            indexAdjustment++;
        }
        ...
        let powerRow2 =
            targetIndex <= 4
                ? gen.powerListTable[targetIndex + 2]
                : gen.powerListTable[gen.powerListTable.length - 3];
        ...
        let powerRow3 =
            targetIndex <= 4
                ? gen.powerListTable[targetIndex + 3]
                : gen.powerListTable[gen.powerListTable.length - 2];
        ...
        const catRow1 = gen.powerCategoriesTable.find(
            (c) => c.name === powerRow1.category,
        );
        const catRow2 = gen.powerCategoriesTable.find(
            (c) => c.name === powerRow2.category,
        );
        const catRow3 = gen.powerCategoriesTable.find(
            (c) => c.name === powerRow3.category,
        );
        ...
        if (char.powers.length === 0 || char.powers[0] === null) {
            Tester.assertEquals(
                "Power Tests",
                char.powers.length,
                `Power Failed to generate.`,
            );
        }

        const p = char.powers[0];
        Tester.assertEquals(
            targetPower.category,
            p.category,
            `Power Category Roll: Generated ${p.category} category.`,
        );
        Tester.assertEquals(
            targetPower.name,
            p.name,
            `Power Roll: Generated ${p.name} name.`,
        );
        Tester.assertEquals(
            "Excellent",
            p.rank,
            `Power Rank Roll: Generated ${p.rank} rank.`,
        );
...
Tester._findPower = (char, entry) => {
    ...
    if (entry.matchBy === "name-or") {
        return char.powers.find(
            (p) => p.name === entry.name || p.name === entry.alt,
        );
    }
...
Tester._validateUltimatePower = (char, targetPower) => {
    ...
    if (hasExtraInfo) {
        Tester.assert(
            p.extraInformation !== "",
            `Power Extra Information Exists: ${p.extraInformation}.`,
        );
    } else {
        Tester.assert(
            p.extraInformation === "",
            `Power Extra Information is blank.`,
        );
    }
    ...
    try {
        ...
        for (const def of bonusDefs) {
            ...
            Tester.assert(
                found != null,
                `If ${targetPower.name}, also has ${def.name || def.alt}.`,
            );
        }
    } catch (ex) {
        Tester.assert(
            false,
            "Exception validating bonus powers for " +
                targetPower.code +
                ": " +
                ex.message,
        );
    ...
    try {
            ...
            if (found) {
                const expectedFlag =
                    def.type === "bonusPower"
                        ...
                        : found.optionalPower;
                const flagType =
                    def.type === "bonusPower" ? "Bonus" : "Optional";
                Tester.assertEquals(
                    true,
                    expectedFlag,
        ...
        Tester.assert(
            false,
            "Exception validating optional powers for " +
                targetPower.code +
                ": " +
                ex.message,
        );
...
Tester.ValidateUltimatePowerCatchTests = () => {
    ...
    ULTIMATE_OPTIONAL_POWERS["EC1"] = new Proxy([{}, {}, {}], {
        get(target, prop) {
            if (typeof prop === "string" && !isNaN(prop) && +prop >= 0)
    ...
    Tester.assert(
        true,
        "ValidateUltimatePowerCatch: validation restored after corruption.",
    );
};
